...
echo "Fetching latest conductor-mayaio-2019 image..."
image_id=$(docker images --digests --no-trunc -q conductor-mayaio-2019:latest)
...
docker run \
   ...
   --env-file="$(pwd)/tests/mayaio-2019/env" \
   $image_id \
   bash -c 'Render \
    -s 1 -e 1 \
    -rd /tmp/output \
